...
- **Wave work (governance vocabulary): built, committed, and human-signed-off.** Wave 1–5 completion reports are committed at `docs/WAVE_1_COMPLETION_REPORT.md` … `docs/WAVE_5_COMPLETION_REPORT.md`; Wave 6 Pieces 1–4 are committed (`8f8760a` … `3143de9`, last landed 2026-07-02). Sign-offs are recorded in the org repo's `review/AUTOPILOT_REVIEW_LOG.md` (verified at HEAD `c9b84d0`): Wave 1–4 completion gates **STOP → SIGNED OFF** (2026-06-29), Wave 5 close-out **SIGNED OFF / PASS by human (Rashed, 2026-06-30)** authorizing Wave 6, Wave 6 pieces **PASS** (2026-07-02), Operator Cockpit UI **PASS by human (Rashed, 2026-07-03)**. The completion baseline is recorded in the org repo's `registry/FACTORY_COMPLETION_PLAN.md` (2026-07-03, human: Rashed).
- **Canonical governance repo:** the GitHub remote `ECE-FACTORY/organization-source-of-truth`; the live local clone is `~/Desktop/ECE FACTORY`. The unversioned snapshots in `~/Downloads` are **non-authoritative** and must not be read as the law.
