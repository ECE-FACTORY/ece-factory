// Build-Decision Seam (Layer 2 — Command) — the deciding→building seam. THE HUMAN'S BUTTON, NOT A BYPASS.
//
// WHAT IT IS: the governed command-layer path that turns an APPROVED harvest proposal into a real
// `ApprovedBuildDecision` the Layer-4 Build Planner can consume. A human picks a spine, records a MEASURED
// air-gap assessment (the one sovereign dimension the harvest machine never measures), and approves the
// promotion at the real Approval Gate. Only then — inside the dispatcher's write handler, where a genuine
// branded `ConsumedApproval` is the sole thing in scope — is the decision assembled.
//
// WHY IT CANNOT SELF-APPROVE (the invariant this file exists to hold):
//   • It MINTS NOTHING. `mintConsumedApproval` / the `APPROVAL_BRAND` symbol are module-private to the bridge
//     (tool-classes.ts:124,136) and are neither imported nor named here. The token type arrives as a TYPE from
//     the governed-adapter CONTRACT — so no value of type ConsumedApproval can be constructed in this module.
...
//     callback — which the dispatcher invokes only on the mint path. No token ⇒ STOP_FOR_APPROVAL ⇒ this seam
//     returns `refused` and produces NOTHING. (Frozen by architecture law Prohibition 4i.)
//
// LAYER 4 STILL "DOES NOT RE-DECIDE": the promotion re-invokes Layer-3's OWN pure `decideSourcing` with the
// human air-gap folded in (never hand-stamps FORK); the planner then reads the decision verbatim.
...
import { foldAirGapMeasurement } from '../../layer-3-harvest/scoring-engine/scoring-engine.js';
...
/** The stable tool name the promotion approval is bound to. */
export const APPROVE_BUILD_DECISION_TOOL = 'approve_build_decision';

/** A human's MEASURED air-gap assessment. `value` is a measurement — never 'unknown'. */
export interface AirGapAssessmentInput {
...
/**
 * PURE. Turn a harvested `SubDomainResult` + a human air-gap assessment into a PROMOTED FORK decision — WITHOUT
 * re-grading and WITHOUT mutating the input. Deep-copies the spine, folds ONLY the air-gap dimension into a
 * fresh score (via the engine's own scale), then re-invokes Layer-3's `decideSourcing` on the completed spine.
 * Legitimate ONLY if that function now returns FORK; otherwise refuses (deny-by-default). The verdict is
 * machine-computed, never hand-stamped.
 */
export function promoteToFork(decision: SubDomainResult, airGap: AirGapAssessmentInput): PromoteResult {
  ...
  // Precondition guard (deny-by-default) — the harvest must already be FORK-eligible MODULO air-gap. These are
  // air-gap-INDEPENDENT facts the machine already established; the seam refuses rather than promote a spine the
  ...
  if (spine.score.total < 70) return { ok: false, stage: 'precondition', reason: `spine normalized score ${spine.score.total} < 70 (not fork-eligible before air-gap)` };
  ...
  // DEEP-COPY the spine; fold ONLY air-gap into a fresh score (other dims byte-for-byte). Original is untouched.
  const promotedScore = foldAirGapMeasurement(spine.score, airGap.value);
  const promotedSpine = {
    ...spine,
    record: { ...spine.record, airGapSuitability: airGap.value },
    score: promotedScore,
  };
  ...
  const promoted: SubDomainResult = {
    ...
    decisionEvidence: [
      ...rederived.evidence,
      `verdict promoted EXTEND→FORK by a human air-gap assessment (value=${airGap.value}); air-gap folded into the score, all other dimensions carried byte-for-byte (no re-grade)`,
    ],
  };
...
export interface PreparedBuildDecision {
  readonly actionId: string;
  readonly binding: ApprovalBinding;
  readonly promoted: SubDomainResult;
  readonly airGap: AirGapAssessmentInput;
...
/**
 * The seam. Two phases with the HUMAN in between:
 ...
 */
export class BuildDecisionSeam {
  ...
  prepare(input: { report: HarvestReport; subDomainKey: string; airGap: AirGapAssessmentInput }): PrepareOutcome {
    ...
    const promo = promoteToFork(sub, input.airGap);
    ...
    const payloadObject = {
      ...
      airGap: input.airGap.value,
    };
    const payloadJson = canonicalPayload(payloadObject);
    const binding: ApprovalBinding = { tool: APPROVE_BUILD_DECISION_TOOL, target, payloadJson };

    const actionId = this.console.enqueue(
      { tool: APPROVE_BUILD_DECISION_TOOL, target, after: payloadObject, risk: 'high', reversible: 'no', requestedBy: { user_id: this.proposingCaller } },
      { tier: 'APPROVAL_REQUIRED_WRITE (build-decision)', blastRadius: 1, proposingCaller: this.proposingCaller },
    );

    return {
      status: 'PENDING-APPROVAL',
      prepared: { actionId, binding, promoted, airGap: input.airGap, sourceReport: { domain: input.report.domain, generatedAtIso: input.report.generatedAtIso } },
    };
  }
   ...
   */
  async assemble(prepared: PreparedBuildDecision): Promise<AssembleOutcome> {
    const { actionId, binding, promoted, airGap, sourceReport } = prepared;
      ...
      'APPROVAL_REQUIRED_WRITE',
      {
        approvalWrite: async (approval: ConsumedApproval): Promise<ApprovedBuildDecision> => {
          ...
          return {
            ...
            approvedBy: approver,
            sourceReport,
            airGapAssessment: { value: airGap.value, rationale: airGap.rationale, measuredBy: approver, gateActionId: actionId },
          };
        },
      },
