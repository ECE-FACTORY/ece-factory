// LIVE end-to-end test for the Harvest Orchestrator — runs the FULL enriched chain for "Identity & Access
// Management" (IAM) against real GitHub (via the committed repo-scout + repo-scout-signals) and writes the real
// Harvest Report to docs/HARVEST_REPORT_IAM.md. The IAM field's proven Apache-2.0 spines (Keycloak, Ory,
// Casbin, Casdoor) are what the sub-domain queries are expected to surface — they are NOT hardcoded here; the
// live scout discovers whatever GitHub ranks for each query, and the real graders decide.
//
...
//
//     GITHUB_TOKEN=xxx npx vitest run src/layer-3-harvest/harvest-orchestrator/harvest-orchestrator-iam.live.test.ts
//     cat docs/HARVEST_REPORT_IAM.md
//
// The builder never supplies or handles the token. This file never hardcodes or prints one. It is a
// domain-parameterized twin of harvest-orchestrator-hr-payroll.live.test.ts — same live wiring, same read-only
// guarantees, only the target domain and the report path differ.
...
const REPORT_PATH = join(__dirname, '..', '..', '..', 'docs', 'HARVEST_REPORT_IAM.md');
...
describe('Harvest Orchestrator — LIVE end-to-end for Identity & Access Management (skips without GITHUB_TOKEN)', () => {
  it.skipIf(!HAS_TOKEN)('runs the full ENRICHED chain for IAM and writes the Harvest Report', async () => {
    ...
    expect(res.reportMarkdown!).toContain('# Harvest Report — Identity & Access Management');
