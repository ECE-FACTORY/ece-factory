...
import { AllowAllAuthorizer } from '../audit-engine/sequencer.js';
  ...
  it('only the PR Engine references the open_pull_request open-path / assembles a PR (src/ scan)', () => {
    ...
    const walk = (dir: string): void => {
      for (const name of readdirSync(dir)) {
        ...
        if (rel.startsWith('features/pr-engine/')) continue; // the PR Engine is the sanctioned assembler
        ...
        if (rel.startsWith('features/mcp-bridge/')) continue;
