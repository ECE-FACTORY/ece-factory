...
describe('External sole-authority — BOUNDARY: exactly ONE owner module references each action’s capability path (9/9)', () => {
  it('each grant<Action>Capability is referenced only by that action’s sanctioned owner module (src/ scan)', () => {
    ...
    const owners: { action: string; marker: RegExp; owner: string }[] = [
      { action: 'create_github_repo', marker: /grantCreateGithubRepoCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'create_ticket', marker: /grantCreateTicketCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'update_crm_record', marker: /grantUpdateCrmRecordCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'send_email', marker: /grantSendEmailCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'deploy_package', marker: /grantDeployPackageCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'create_milestone', marker: /grantCreateMilestoneCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'create_label', marker: /grantCreateLabelCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'create_issue_batch', marker: /grantCreateIssueBatchCapability\(/, owner: 'layer-5-action/external-gateways/' },
      { action: 'open_pull_request', marker: /grantPrOpenCapability\(|grantOpenPullRequestCapability\(/, owner: 'layer-5-action/pr-engine/' },
    ];
    const files: string[] = [];
    ...
    for (const { action, marker, owner } of owners) {
      ...
      const offenders = refs.filter((rel) => !rel.startsWith('layer-5-action/mcp-bridge/') && !rel.startsWith(owner));
