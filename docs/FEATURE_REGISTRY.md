...
| Feature | Path | Module | Status | Feature file |
|---------|------|--------|--------|--------------|
| Tier-Status Health Check | `src/mcp-server/tier-status.ts` | — (Phase 9.2, OPEN_ITEM #10) | **Built & tested** — READ-ONLY operational reporter of each MCP tier's backing (live/fake/disabled/not-wired), **derived from the real injected adapter via `instanceof`, not a label** (a fake can never be reported as live); reports DB reachability (read-only probe) + core-table/migration count + DB role names + Claude-Code-registration as `unknown/external`; no side effects, no secrets in output; `health` JSON-RPC method + `npm run mcp:healthz`; composition-root | [tier-status.feature.md](../src/mcp-server/tier-status.feature.md) |
